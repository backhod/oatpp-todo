...
public:
    ...
    ENDPOINT("GET", "todos/{todoId}", getTodoById,
             PATH(Int32, todoId))
    {
    ...
    {
        info->summary = "get all stored todos";

        info->addResponse<oatpp::Vector<oatpp::Object<TodoDto>>>(Status::CODE_200, "application/json");
        info->addResponse<Object<StatusDto>>(Status::CODE_500, "application/json");
    }
    ...
    ENDPOINT("DELETE", "todos/{todoId}", deleteTodo,
             PATH(Int32, todoId))
    {
        return createDtoResponse(Status::CODE_200, m_todoService.deleteTodo(todoId));
    }
};
